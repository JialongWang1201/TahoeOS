...
void HardwareInitTask(void *argument)
{
    ...
    // EEPROM
    EEPROM_Init();
    uint8_t app_data_defaulted = AppData_Load();
    if(!EEPROM_Check())
    {
      uint8_t recbuf[3];
      if(app_data_defaulted)
      {
        SettingGet(recbuf,0x10,2);
        if((recbuf[0] == 0 || recbuf[0] == 1) && (recbuf[1] == 0 || recbuf[1] == 1))
        {
          g_app_data.wrist_enabled = recbuf[0];
          g_app_data.app_sync_enabled = recbuf[1];
        }
      }
    ...
    HWInterface.IMU.wrist_is_enabled = g_app_data.wrist_enabled;
    ui_APPSy_EN = g_app_data.app_sync_enabled;
    ui_LTimeValue = g_app_data.light_time_seconds;
    ui_TTimeValue = g_app_data.turnoff_time_seconds;
...

    {
      ...
      uint8_t health_day_state;
      HWInterface.RealTimeClock.GetTimeDate(&nowdatetime);
      health_day_state = AppData_EnsureHealthDay(nowdatetime.Year, nowdatetime.Month, nowdatetime.Date, nowdatetime.WeekDay);
      if(app_data_defaulted || health_day_state != APPDATA_HEALTH_DAY_UNCHANGED)
      {
        AppData_Save();
      }
    }

	...
	while(1)
	{
		...
		if(osMessageQueueGet(DataSave_MessageQueue,&Datastr,NULL,1)==osOK)
		{
			...
			****************/
            ...
            g_app_data.wrist_enabled = HWInterface.IMU.wrist_is_enabled;
            g_app_data.app_sync_enabled = ui_APPSy_EN;
            g_app_data.light_time_seconds = ui_LTimeValue;
            g_app_data.turnoff_time_seconds = ui_TTimeValue;
            AppData_Save();

...
lv_obj_t * ui_RemoteAlertPage;
lv_obj_t * ui_RemoteAlertTitle;
lv_obj_t * ui_RemoteAlertLabel;
lv_obj_t * ui_RemoteAlertButton;
lv_obj_t * ui_RemoteAlertButtonLabel;
//power slider
...
Page_t Page_RemoteAlert = {ui_RemoteAlertPage_screen_init, ui_RemoteAlertPage_screen_deinit, &ui_RemoteAlertPage};

static char ui_HomeWeatherSummary[24];
static char ui_RemoteAlertTitleText[APPDATA_NOTIFY_TITLE_MAX] = "Find Watch";
static char ui_RemoteAlertMessageText[APPDATA_NOTIFY_BODY_MAX] = "Phone requested attention";

static void ui_RemoteAlertApplyContent(void)
{
    if(ui_RemoteAlertTitle != NULL)
    {
        lv_label_set_text(ui_RemoteAlertTitle, ui_RemoteAlertTitleText);
    }

    if(ui_RemoteAlertLabel != NULL)
    {
        lv_label_set_text(ui_RemoteAlertLabel, ui_RemoteAlertMessageText);
    }
}
...
static void HomePage_timer_cb(lv_timer_t * timer)
{
  ...
  char weather_buf[sizeof(ui_HomeWeatherSummary)];
  if(Page_Get_NowPage()->page_obj  == &ui_HomePage)
	{
      ...
      if(AppRuntime_BuildWeatherSummary(weather_buf, sizeof(weather_buf)))
      {
        if(strcmp(ui_HomeWeatherSummary, weather_buf))
        {
          strcpy(ui_HomeWeatherSummary, weather_buf);
          lv_label_set_text(ui_WeatherLabel, ui_HomeWeatherSummary);
        }
      }
      else if(strcmp(ui_HomeWeatherSummary, "No sync"))
      {
        strcpy(ui_HomeWeatherSummary, "No sync");
        lv_label_set_text(ui_WeatherLabel, ui_HomeWeatherSummary);
      }
...
void ui_event_NFCButton(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_VALUE_CHANGED &&  lv_obj_has_state(target, LV_STATE_CHECKED))
    {
        ...
        if(slot == APPDATA_NFC_SLOT_NONE)
        {
            ui_HomePageNFCEN = 0;
            g_app_data.nfc_enabled = 0;
            lv_obj_clear_state(target, LV_STATE_CHECKED);
            ui_mbox_create((uint8_t *)"No card");
            User_RequestDataSave();
            return;
        }

        ui_HomePageNFCEN=1;
        g_app_data.active_nfc_slot = slot;
...
void ui_event_RemoteAlertButton(lv_event_t * e)
{
...
void ui_HomePage_screen_init(void)
{
		ui_MenuScrollY = 0;
    uint8_t value_strbuf[10];
    ui_HomePageNFCEN = g_app_data.nfc_enabled;
    if(ui_HomePageNFCEN && !AppData_IsValidNfcSlot(g_app_data.active_nfc_slot))
    {
    ...
    memset(ui_HomeWeatherSummary, 0, sizeof(ui_HomeWeatherSummary));
    if(!AppRuntime_BuildWeatherSummary(ui_HomeWeatherSummary, sizeof(ui_HomeWeatherSummary)))
    {
        strcpy(ui_HomeWeatherSummary, "No sync");
    }

    ui_WeatherLabel = lv_label_create(ui_HomePage);
    lv_obj_set_width(ui_WeatherLabel, 120);
    lv_obj_set_height(ui_WeatherLabel, LV_SIZE_CONTENT);
    lv_obj_set_x(ui_WeatherLabel, -20);
    lv_obj_set_y(ui_WeatherLabel, -88);
    lv_obj_set_align(ui_WeatherLabel, LV_ALIGN_RIGHT_MID);
    lv_label_set_text(ui_WeatherLabel, ui_HomeWeatherSummary);
    lv_label_set_long_mode(ui_WeatherLabel, LV_LABEL_LONG_DOT);
    lv_obj_set_style_text_color(ui_WeatherLabel, lv_color_hex(0x808080), LV_PART_MAIN | LV_STATE_DEFAULT);
...
void ui_RemoteAlertPage_screen_init(void)
{
    ui_RemoteAlertPage = lv_obj_create(NULL);
    lv_obj_clear_flag(ui_RemoteAlertPage, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_style_bg_color(ui_RemoteAlertPage, lv_color_hex(0x14213D), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_RemoteAlertPage, 255, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_RemoteAlertTitle = lv_label_create(ui_RemoteAlertPage);
    lv_obj_set_align(ui_RemoteAlertTitle, LV_ALIGN_TOP_MID);
    ...
    lv_obj_set_align(ui_RemoteAlertLabel, LV_ALIGN_CENTER);
    lv_obj_set_y(ui_RemoteAlertLabel, -10);
    lv_label_set_text(ui_RemoteAlertLabel, ui_RemoteAlertMessageText);
    ...
    lv_obj_set_height(ui_RemoteAlertButton, 46);
    lv_obj_set_align(ui_RemoteAlertButton, LV_ALIGN_BOTTOM_MID);
    lv_obj_set_y(ui_RemoteAlertButton, -24);
    lv_obj_clear_flag(ui_RemoteAlertButton, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_style_bg_color(ui_RemoteAlertButton, lv_color_hex(0xFCA311), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_RemoteAlertButton, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void ui_RemoteAlertPage_screen_deinit(void)
{
    ui_RemoteAlertPage = NULL;
    ui_RemoteAlertTitle = NULL;
    ui_RemoteAlertLabel = NULL;
    ui_RemoteAlertButton = NULL;
    ui_RemoteAlertButtonLabel = NULL;
}

void ui_RemoteAlertShow(const char *title, const char *message)
{
    Page_t *now_page;

    memset(ui_RemoteAlertTitleText, 0, sizeof(ui_RemoteAlertTitleText));
    memset(ui_RemoteAlertMessageText, 0, sizeof(ui_RemoteAlertMessageText));

    if(title != NULL && title[0] != '\0')
    {
        strncpy(ui_RemoteAlertTitleText, title, sizeof(ui_RemoteAlertTitleText) - 1u);
    }
    else
    {
        strncpy(ui_RemoteAlertTitleText, "Notification", sizeof(ui_RemoteAlertTitleText) - 1u);
    }

    if(message != NULL && message[0] != '\0')
    {
        strncpy(ui_RemoteAlertMessageText, message, sizeof(ui_RemoteAlertMessageText) - 1u);
    }
    else
    {
        strncpy(ui_RemoteAlertMessageText, "Phone requested attention", sizeof(ui_RemoteAlertMessageText) - 1u);
    }

    ui_RemoteAlertApplyContent();
    now_page = Page_Get_NowPage();

    if(now_page != NULL && now_page->page_obj == &ui_RemoteAlertPage)
    {
        return;
    }

    Page_Load(&Page_RemoteAlert);
}

void ui_RemoteAlertTrigger(void)
{
    ui_RemoteAlertShow("Find Watch", "Phone requested attention");
}

void ui_RemoteAlertDismiss(void)
{
...
uint8_t ui_RemoteAlertIsActive(void)
{
    ...
    return now_page->page_obj == &ui_RemoteAlertPage;
}
